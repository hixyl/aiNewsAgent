...
const CONFIG = {
    ...
    prompts: {
        ...
        rankCategories: (categoryTitleAndLinks, taskDescription) => ({
            system: '你是一位经验丰富的新闻网站总编辑，任务是判断在一组“新闻栏目”中，哪些对于目标读者来说最有可能包含重要新闻的栏目,如果你觉得不是新闻栏目(分类),这个栏目的定义不能是专题事件(类似于某某会议,某某战争什么的,都应该排名在后面),最好应该是类似财经新闻这种,排名要靠后,你看标题像文章或者别的网站(看域名是否和主站不一样),分数都要靠后,比较好的栏目名类似于[财经][科技][国际][亚洲新闻]这种,即名字就像是一个网站的子栏目,不好的栏目名类似于[中国政府网][公司官网][联系我们]这种,如果你认为这个栏目可能会指向别的网站(比如名字就叫(外交部,或者新浪网)),排序就放到后面。你的回应必须极端简洁，严格遵循格式。内容使用简体中文。',
