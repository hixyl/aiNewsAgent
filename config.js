...
`
       ...
       // 聚类第二步的“一对一”验证
        verifySingleArticleConsistency: (theme, articleTitle) => ({
            system: '你是一个逻辑判断机器人，任务是判断一个新闻标题是否与给定的核心议题完全相关。你的回应必须是单个词。',
            user: `**核心议题**: "${theme}"

**待检验的新闻标题**: "${articleTitle}"

**你的指令**:
判断“待检验的新闻标题”是否与“核心议题”报道的是同一件事。
- 如果是，回应 "yes"。
- 如果不是，回应 "no"。
...
**你的回应**:`,
        ...
        // (新增) 聚类第一阶段：进行广泛的、主题驱动的聚类
        initialBroadClustering: (articles) => ({
            system: '你是一个高效的新闻议题聚合引擎。你的任务是快速地将一个文章标题列表按照它们报道的核心新闻事件进行分组。你的输出必须是一个纯粹的、格式完美的JSON对象。',
            user: `
**任务**: 阅读下面的“文章列表”，并将报道【同一核心事件】的文章ID聚合在一起。

**文章列表**:
${articles.map((art, i) => `ID_${i}: "${art.title}"`).join('\n')}

**你的指令**:
1.  通读所有文章标题，理解它们各自的核心内容。
2.  将描述同一事件（即使角度、细节或措辞不同）的文章ID分组。
3.  你的回应【必须且只能】是一个JSON对象。
4.  JSON的【key】是议题的临时名称（例如 "topic_1", "topic_2"），【value】是一个包含该议题所有文章ID（数字）的数组。
5.  **重要**: 宁可错分，也不要漏分。如果两篇文章看起来“可能”相关，就先把它们分到一组。后续步骤会进行验证。
6.  只包含成员数大于1的议题。完全不相关的文章无需在结果中体现。
