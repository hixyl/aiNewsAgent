...
export async function generateFinalReport(articles, dailyOutputDir, spinner) {
    ...
    // (核心修改) 根据新增的 finalScore 进行最终排序
    const sortedArticles = _.orderBy(articles, ['finalScore'], ['desc']);
    logger.info(`已根据最终综合重要性得分对 ${articles.length} 篇成功处理的议题完成排序。`);
    ...
    for (const [index, article] of sortedArticles.entries()) {
        ...
        finalMarkdown += `* **发布日期**: ${article.publishedDate || '未知'}\n`;
        finalMarkdown += `* **综合得分**: ${article.finalScore} (锦标赛: ${article.tournamentScore}, 新颖度: ${article.recencyBonus})\n`;
        ...
        if (isCluster) {
            finalMarkdown += `**相关原文链接 (${article.clusterSize}篇)**:\n`;
